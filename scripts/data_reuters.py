...
df = pd.read_json(args.input, orient='records', lines=True, dtype=object)
all_timestamps_ini = []
all_docs_ini = []
all_topic_ids = []
line_count = 0
for i in df.itertuples():
    text = re.sub(http_regex, '', i.body).strip()
    text = re.sub(alphanumeric_regex, '', text)
    all_docs_ini.append(text)
    all_timestamps_ini.append(str(i.date.strftime("%Y-%m-%d")))
...
# Write as raw text
out_filename = './' + fname + '_all_docs_splitParagraphs' + str(flag_split_by_paragraph) + '.txt'
...
if split_sets:
    ...
    path_save = './' + fname + '/split_paragraph_' + str(flag_split_by_paragraph) + '/min_df_' + str(min_df) + '/'
    ...
    path_save = './' + fname + '/all_docs_split_paragraph_' + str(flag_split_by_paragraph) + '/min_df_' + str(min_df) + '/'
    if not os.path.isdir(path_save):
        os.system('mkdir -p ' + path_save)
